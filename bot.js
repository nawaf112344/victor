...
client.user.setGame(`الأوامر الملكيه 📢`,"http://twitch.tv/S-F")
...
.setFooter("-")
 ...
 client.on('message',async message => {
  ...
  let acRoom = client.channels.get('558756510625169459');
  if(message.content.startsWith(prefix + "قبول")) {
    if(message.guild.id !== '558756510625169459') return;
    ...
    mention.addRole(mySupport).then(() => {
      acRoom.send(`**:white_check_mark: [ ${mySupport} ] واعطائك رتبة ${mention} تم بنجاح قبولك**`);
    });
  }
});
...
.then(co => {
  md = co.first().content
        co.first().delete()
        e.edit(`**خبرتك بالدسكورد**`)
let br = message.channel.awaitMessages(filter, { max: 1, time: 40000, errors: ['time'] })
.then(col => {
  br = col.first().content
        col.first().delete()
e.edit("**شكراً على تقديمك سوفا رد عليك بأسرع وقت ...**").then(b => {
        setTimeout(() => {
